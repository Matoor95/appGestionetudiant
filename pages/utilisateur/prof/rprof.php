...
{
	$rechercher=$_POST[rechercher];
$req=$bd->query("select nom,prenom,tel,email,photo from PROFESSEUR where prenom like '%".$rechercher."%' or nom like '%".$rechercher."%' order by nom,prenom"); 

// IL PERMET DE VOIR SI ON A UN RESULTAT SI C OUI 
$req2=$bd->query("select nom,prenom,tel,email,photo from PROFESSEUR where prenom like '%".$rechercher."%' or nom like '%".$rechercher."%' order by nom,prenom"); 

}
else
{
$req=$bd->query("select nom,prenom,tel,email,photo from professeur order by nom,prenom"); 
$req2=$bd->query("select nom,prenom,tel,email,photo from professeur order by nom,prenom"); 
